...
boolean CCS811::begin(uint8_t I2C_ADDR, uint8_t WAKE_PIN)
{
  ...
  {
    SerialUSB.println("Error: Incorrect Hardware ID detected.");
    return false;
  }
  ...
  if(bit != 1)
  {
    SerialUSB.println("Error: No application firmware loaded.");
    readErrorID(status);
    return false;
  }
  ...
  if(bit != 1)
  {
    SerialUSB.println("Error: Firmware still in boot mode.");
    readErrorID(status);
    return false;
  }
  ...
  Wire.write(0x10);  // 0x20: IAQ measurement every 10 seconds
  Wire.endTransmission();
  //CCS811::sleep();
  digitalWrite(_WAKE_PIN, HIGH);
...
byte CCS811::readErrorID(byte _status)
{
  ...
  if(bit == 1)
  {
    SerialUSB.print("Error ID: ");
    SerialUSB.println(error_id);
  }
...
void CCS811::sleep()
{
  ...
  Wire.beginTransmission(_I2C_ADDR);
  Wire.write(MEAS_MODE);
  Wire.write(0x00);  // sets sensor to idle; measurements are disabled; lowest power mode
  Wire.endTransmission();
